...
@Component({ selector: 'app-reports-page', standalone: true, imports: [FormsModule], templateUrl: './reports.html' })
export class ReportsPageComponent {
  ...
  get activeClients(): number { return this.data.clients.filter(item => item.status === 'Activo').length; }
  generate(): void { this.ready = true; this.generatedAt = new Date().toLocaleString('es-EC', { dateStyle: 'medium', timeStyle: 'short' }); }
  download(): void { const rows = [['Indicador','Valor'],['Ingresos',this.paidTotal.toFixed(2)],['Clientes activos',String(this.activeClients)],['Asistencias',String(this.data.attendance.length + 128)],['Inventario',String(this.data.supplements.length + this.data.machines.length)]]; const blob = new Blob([rows.map(row => row.join(',')).join('\n')], { type: 'text/csv' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'reporte-fit-admin.csv'; link.click(); URL.revokeObjectURL(link.href); }
}
